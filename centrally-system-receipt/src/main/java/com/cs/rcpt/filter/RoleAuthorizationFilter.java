...
/**
 * ① receipt.role.exclude.url 에 등록된 엔드포인트는 그냥 통과  
 * ② 그 외 요청은 JWT roles 중 receipt.roles 교집합이 있는지 검사
 */
...
public class RoleAuthorizationFilter extends OncePerRequestFilter {
    ...
    /** RECEIPT 서비스가 요구하는 권한 목록 */
    private final List<String> receiptRoles;
    ...
    protected void doFilterInternal(HttpServletRequest  request,
        ...
        boolean allowed = userRoles.stream().anyMatch(receiptRoles::contains);
    ...
    private boolean isExcluded(HttpServletRequest req) {
        ...
        Integer jwtUserId     = extractUserId(auth);   // 🔹 JWT/Principal에서 userId 꺼냄
    ...
    /** Authentication(Principal)에서 userId 꺼내기 */
    private Integer extractUserId(Authentication auth) {
        if (auth instanceof UsernamePasswordAuthenticationToken token) {
            ...
            if (principal instanceof com.cs.core.security.CustomUserPrincipal user) {
                return user.getUserId(); // 반드시 getUserId() 가 존재해야 함
            }
        }
        return null;
    }

...
public class ReceiptService {
    ...
    public Receipt createUserReceipt(Integer userId,
        ...
        // 포트폴리오용 - 구현 제거됨
        // 기능: 영수증 생성 처리
        // - 영수증 코드 자동 생성 (YYMM-번호 형식, 월별 시퀀스, Pessimistic Lock)
        // - 기본 정보 설정 (사용자, 날짜, 카테고리, 금액, 사유)
        // - 참여자 JSON 파싱 및 설정
        // - 결재선 JSON 파싱 및 설정 (역할 코드 계산 포함)
        // - 첫 미결재 단계(currentApprovalStep) 자동 계산
        // - 첨부파일 저장 및 이미지 처리
...
//                    line.setRejectedReason(null);    // 반려 사유 제거
    ...
    private ReceiptAttachments storeAttachment(MultipartFile file, Receipt owner) {
        // 포트폴리오용 - 구현 제거됨
        // 기능: 영수증 첨부파일 저장 및 이미지 처리
        // - 파일명 안전화 처리
        // - 이미지 압축 및 리사이즈 처리 (ImageProcessor 사용)
        // - 파일 저장 경로 설정
    ...
    private List<ReceiptApprovalLine> parseApprovers(String json, Receipt owner) {
        // 포트폴리오용 - 구현 제거됨
        // 기능: 결재선 JSON 파싱 및 엔티티 생성
        // - JSON 파싱 (ObjectMapper 사용)
        // - 결재 역할 코드 계산 로직 (기본결재자=3, 합의=2, 결재=1)
        // - stepOrder 자동 할당 (1부터 순차적으로)
        // - ReceiptApprovalLine 엔티티 생성 및 매핑
        throw new RuntimeException(GlobalExceptionHandler.CC + "포트폴리오용으로 구현이 제거되었습니다.");
    }
}

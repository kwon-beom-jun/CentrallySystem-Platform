...
import com.cs.rcpt.filter.RoleAuthorizationFilter;
import com.cs.rcpt.filter.RoleAuthorizationFilter.ExcludeEntry;
...
public class SecurityConfig {
    ...
    public RoleAuthorizationFilter roleAuthorizationFilter(
            @Value("${receipt.roles}")             String receiptRoles,
            @Value("${receipt.role.exclude.url:}") String excludeUrls,
            AccessDeniedHandler jsonAccessDeniedHandler) {

        List<String> roles = Arrays.stream(receiptRoles.split(","))
                                   .map(String::trim)
                                   .filter(s -> !s.isEmpty())
                                   .toList();

...
#### 🌐 **API 엔드포인트 (AuthLoginController.java)**
```java
...
#### 📡 **API 클라이언트 (LoginApi.js)**
- ✅ `refreshToken()`: Refresh Token으로 갱신
- ✅ `logout()`: 로그아웃
- ✅ `logoutAll()`: 모든 기기 로그아웃

#### 🔄 **Axios Interceptor (apiConfig.js)**
```javascript
// 401 에러 시 자동 처리 흐름
...
```bash
# Flyway 또는 수동 실행
psql -U postgres -d centrally_db -f V1__Create_Auth_Refresh_Tokens_Table.sql
```

...
- **기본 디렉토리**
  ...
  - 프론트 dist:
    - `/was/censys/dist` (Nginx `root` 설정이 이 경로)
- **서비스별 포트** (Config `resources-dev-linux` 기준)
  ...
  - Receipt: `9899`
- **배포 스크립트** (Jenkins가 호출)
  - `/was/censys/deploy-service.sh`
  - 역할 (핵심 요약)
    1. `<서비스명>.jar` 실행 중 프로세스 찾기 → 종료 (우선 SIGTERM, 필요 시 SIGKILL)
    2. `/was/censys/<서비스명>.jar` 새로 실행 (nohup)
    3. `http://localhost:<포트>/actuator/health`로 헬스체크
    4. 로그는 `/was/censys/logs/<서비스명>.log` 에 쌓임
...
- **로그 조금만 보고 싶을 때**
  - `tail -50 /was/censys/logs/auth.log`
  - `tail -50 /was/censys/logs/gateway.log`
...
- Jenkins는 **변경된 서비스만 빌드**하고,  
  ...
  **AP 서버 `/was/censys`로 복사 + `deploy-service.sh` 실행**까지 담당합니다.
- AP 서버에서는 모든 것이 **`/was/censys` 폴더 안**에서 돌아가며,  

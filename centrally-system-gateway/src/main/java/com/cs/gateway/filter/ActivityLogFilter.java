...
public class ActivityLogFilter implements WebFilter {
    ...
    private Mono<Void> saveLogAndContinue(ServerWebExchange exchange, WebFilterChain chain,
        ...
        String browser = UserAgentParser.getBrowserString(userAgent);
        ...
        log.debug(">>> ActivityLogFilter: email={}, name={}, menu={}, func={}, path={}, method={}, browser={}, body={}",
                userEmail, username, menu, func,
                exchange.getRequest().getURI().getPath(), method, browser, bodyString);
     ...
     *    - 예: {"userEmail": "test@example.com", "password": "dummy_password_1234"}
     * 
     * 2. 본문 재공급 준비
                    ...
                    public Mono<Void> writeWith(org.reactivestreams.Publisher<? extends DataBuffer> body) {
                        ...
                        if (statusCode != null && statusCode.is2xxSuccessful()) {
                            ...
                            userActivityLogDao.saveLog(logEntity).subscribe(
                                saved -> log.info(">>> [DB 저장 성공] 로그인 성공 이력: email={}, name={}, browser={}", 
                                        userEmailRef.get(), usernameRef.get(), browser),
                        ...
                        // ──────────────────────────────────────────────────────────
                            ...
                            // DB에 비동기 저장
                            userActivityLogDao.saveLog(logEntity).subscribe(
                                saved -> log.info(">>> [DB 저장 성공] 로그인 실패 이력: email={}, 상태={}, browser={}", 
                                        userEmailFromBody, statusCode.value(), browser),

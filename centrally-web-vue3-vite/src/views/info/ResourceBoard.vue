<template>
  <div>
    ...
    <div class="content content-wrapper">
      <PageTitle 
        :title="$t('info.board.title.resource')"
        subtitle="Resource Board"
        icon="ri-folder-line"
      />
...
function onPageChange(newPage) {
  ...
  viewState.saveState('infoResourceBoard', {
...
async function fetchDataFromServer(page = 1, restoreScroll = false) {
  ...
  const adjusted = await adjustPageIfExceedsTotal({
    ...
    viewState,
    stateKey: 'infoResourceBoard',
    fetchFunction: fetchDataFromServer,
  });
  if (adjusted) return;

  if (restoreScroll) {
    const saved = viewState.getState('infoResourceBoard');
...
function showModal() {
  ...
  if (isMobile.value) {
    ...
    router.push(ROUTES.INFO.POST_CREATE_MOBILE + '?boardCode=RESOURCE');
  } else {
    isCreate.value = true;
...
function goToDetail(item) {
  ...
  router.push({
    ...
    query: { postId: item.id, boardCode: 'RESOURCE' }
  });
}
...
onMounted(async () => {
  const boardResponse = await BoardApi.getBoardByCode(BOARD_CODE.RESOURCE);
  boardId.value = boardResponse.data.id;

  // MANAGER 이상 작성 가능
  canWrite.value = authStore.roles.includes('ROLE_INFO_MANAGER') || authStore.roles.includes('ROLE_INFO_ADMIN');

  const saved = viewState.getState('infoResourceBoard');
  const restore = viewState.canRestore('infoResourceBoard');
  if (restore && saved) {
  ...
  viewState.allowRestoreFrom('infoResourceBoard', ['InfoPostDetail', 'InfoPostCreateMobile', 'InfoPostEditMobile']);
});

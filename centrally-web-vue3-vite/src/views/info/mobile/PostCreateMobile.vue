  ...
  <div>
    <div class="content content-wrapper">
      ...
      <form class="form-pane modal-body" @submit.prevent="onSave">
        ...
        <div class="form-group cs-card--blue" v-if="canAdmin && boardCode !== BOARD_CODE.COMMUNITY">
          <div class="mail-card">
            ...
            <div v-show="sendMail" class="mail-card__body">
              <div class="row-top">
                <div class="segmented">
                  ...
                  <button type="button" :class="['seg-btn', mailTargetMode === 'department' && 'active']" @click="mailTargetMode = 'department'">{{ $t('info.postMobile.mail.department') }}</button>
                  <button type="button" :class="['seg-btn', mailTargetMode === 'team' && 'active']" @click="mailTargetMode = 'team'">{{ $t('info.postMobile.mail.team') }}</button>
                </div>
                <div class="emp-chips">
        ...
        <div class="form-group cs-card--blue" v-if="canAdmin">
          ...
          <ul v-if="newFiles.length" class="attachment-list">
            <li v-for="(file, idx) in newFiles" :key="`n-${idx}-${file.name}`" class="attachment-item">
              <span class="attachment-badge new">{{ $t('info.postMobile.file.new') }}</span>
              <span class="attachment-name">{{ file.name }}</span>
              <button type="button" class="btn btn-danger square-btn" @click="removeNewFile(idx)">x</button>
            </li>
          </ul>
        </div>
        ...
        <DefaultFormRow align="right" marginTop="10px">
          ...
          <MobileMintButton type="submit" style="margin-left:8px">{{ $t('info.postMobile.buttons.create') }}</MobileMintButton>
        </DefaultFormRow>
      </form>
    </div>
  </div>
...
watch(() => mailTargetMode.value, () => { resetMailSelections(); });
watch(() => sendMail.value, (isOn) => { if (isOn) initializeMailPanel(); else resetMailSelections(); });
...
async function onSave() {
  ...
  if (newId) {
    router.replace(ROUTES.INFO.POST_DETAIL + '?postId=' + newId + '&boardCode=' + boardCode.value);
  } else {
    const routes = {
      'NOTICE': ROUTES.INFO.NOTICE,
      'RESOURCE': ROUTES.INFO.RESOURCE,
...
onMounted(async () => {
  ...
  Promise.all([DepartmentApi.getDepartments(), TeamApi.getTeams()]).then(([deptRes, teamRes]) => {
...
/* 메일 카드 */
.mail-card { border: 1px solid #e5e7eb; border-radius: 10px; padding: 10px; background: #fff; }
.mail-card__header { display: flex; justify-content: space-between; align-items: center; }
.mail-card__title { display: inline-flex; align-items: center; gap: 8px; font-weight: 600; color: #374151; font-size: 0.8rem; }
...
.seg-btn { border: 1px solid #d1d5db; background: #f9fafb; color: #374151; border-radius: 999px; padding: 5px 9px; font-size: 0.7rem; }
.seg-btn.active { background: #3b82f6; color: #fff; border-color: #3b82f6; }
.emp-chips { display:flex; gap:6px; flex-wrap: wrap; }
.chip { border: 1px solid #d1d5db; background: #fff; color: #374151; border-radius: 999px; padding: 4px 8px; font-size: 0.7rem; }

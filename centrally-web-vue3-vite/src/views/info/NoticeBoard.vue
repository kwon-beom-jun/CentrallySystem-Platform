<template>
  <div>
    ...
    <div class="content content-wrapper">
      <PageTitle 
        :title="$t('info.board.title.notice')"
...
function onPageChange(newPage) {
  ...
  viewState.saveState('infoNoticeBoard', {
...
async function fetchDataFromServer(page = 1, restoreScroll = false) {
  ...
  if (restoreScroll) {
    const saved = viewState.getState('infoNoticeBoard');
...
function showModal() {
  ...
  if (isMobile.value) {
    ...
    router.push(ROUTES.INFO.POST_CREATE_MOBILE + '?boardCode=NOTICE');
  } else {
    isCreate.value = true;
...
function goToDetail(item) {
  ...
  router.push({
    ...
    query: { postId: item.id, boardCode: 'NOTICE' }
  });
}
...
onMounted(async () => {
  // 게시판 ID 조회
  const boardResponse = await BoardApi.getBoardByCode(BOARD_CODE.NOTICE);
  boardId.value = boardResponse.data.id;

  // 권한 확인 (ADMIN만 작성 가능)
  canWrite.value = authStore.roles.includes('ROLE_INFO_ADMIN');

  const saved = viewState.getState('infoNoticeBoard');
  const restore = viewState.canRestore('infoNoticeBoard');
  if (restore && saved) {
  ...
  viewState.allowRestoreFrom('infoNoticeBoard', ['InfoPostDetail', 'InfoPostCreateMobile', 'InfoPostEditMobile']);
});

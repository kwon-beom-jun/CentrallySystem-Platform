<template>
  <div>
    ...
    <div class="content content-wrapper" :class="{ 'content-expanded': sidebarVisible && !isMobile }">
      <PageTitle
        :title="`${userName}${$t('receipt.common.detailTitleConcurrence')}`"
      ...
      <div v-else-if="data.length > 0">
        <div class="d-flex align-items-center justify-content-end content-body-header">
          <span class="mr-2 all-click" @click="onSelectAllClick('2', $event)">{{ $t('receipt.common.allAgree') }}</span>
          <DefaultTextfield
            type="radio"
            name="approveAll" 
            class="approveAll" 
...
function onDecisionRadioClick(row, value, e) {
  ...
  if (current === targetValue) {
    row.decision = ''
    if (targetValue === '3') row.rejectReason = ''
  } else {
    row.decision = targetValue
    if (targetValue === '2') row.rejectReason = ''
  }
  updateDecision()
}

/* 전체 합의/반려 라디오도 같은 값 재클릭시 해제 */
function onSelectAllClick(value, e) {
  ...
  if (isSame) {
    if (v === '2') {
      data.value.forEach(r => { if (r.decision == 2 || r.decision === '2') r.decision = '' })
    } else if (v === '3') {
      data.value.forEach(r => {
...
async function save() {
  ...
  if (!payload.length) return toast.warning('합의 또는 반려된 항목이 없습니다.')
...
@media (max-width: 650px) {
  ...
  /* 2. 왼쪽 스텁 제거 */
  .receipt-card::before { content: none !important; }

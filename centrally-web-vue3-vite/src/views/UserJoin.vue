...
async function handleSubmit() {
  ...
  // RSA 공개키 조회
  const publicKeyResponse = await LoginApi.getPublicKey();
  const publicKey = publicKeyResponse.data.publicKey;
  
  // 비밀번호 암호화
  const encryptedPassword = RsaEncryptionUtil.encryptPassword(password.value, publicKey);
  
  const response = await TempUsersApi.createTempUser({
    ...
    agreementLanguageCode: locale.value,
    agreements: agreements.value.map((a) => ({
      agreementId: a.agreementId,
      isAgreed: a.checked,
    })),
  });
  
  toastStore.setNextPageMessage(response.data.message);
  onGoBack();

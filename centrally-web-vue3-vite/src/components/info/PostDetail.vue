...
function setPostAuthorProfile() {
  ...
  post.value.authorProfileImg = { fileUrl: author.profileImgUrl };
...
async function deletePost() {
  ...
  const stateKey = `info${boardCode.value}Board`;
  ...
  const shouldGoToPrevPage = currentPage > 1 && (!response.data.content || response.data.content.length === 0);
...
function getAvatarUrl(userId) {
  ...
  return user?.profileImgUrl || authorImgUrl.value || '';
}

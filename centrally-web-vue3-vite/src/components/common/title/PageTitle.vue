  ...
  <div class="page-title-wrapper" :class="{ 'has-back-button': showBackButton }" ref="wrapperRef">
    <h2 class="content-title">
      ...
      <span class="page-title-text">
        <i v-if="icon" :class="icon" class="page-title-icon"></i>
        {{ title }}
      </span>
    </h2>
    <div 
...
const handleScroll = () => {
  ...
  if (!isScrolled.value && scrollY > 37) {
    // 내릴 때: 37px 넘으면 사라짐
    wrapperRef.value.classList.add('scrolled')
    isScrolled.value = true
  } else if (isScrolled.value && scrollY < 5) {
    // 올릴 때: 5px 아래로 내려오면 나타남
...
@media (max-width: 650px) {
  .page-title-padding {
    padding-bottom: var(--mobile-padding-bottom, 40px) !important;
  }
  .page-title-wrapper {
    position: sticky;
    top: 64px;
...
/* 다크모드: 서브타이틀 색상 밝게 */
body[data-theme="dark"] .content-sub-title {
  color: #d1d5db !important;
}

/* 다크모드: 스크롤 시 타이틀 배경 */
@media (max-width: 650px) {
  body[data-theme="dark"] .page-title-wrapper.scrolled {
    background-color: var(--theme-bg-main) !important;
    border-bottom: 1px solid var(--theme-border) !important;
    box-shadow:

...
const drawerUser = computed(() => ({
  name: hrmStore.getMyProfile?.name ?? '',
  email: hrmStore.getMyProfile?.email ?? '',
  position: hrmStore.getMyProfile?.positionName ?? '',
  photoUrl: hrmStore.getMyProfile?.profileImgUrl ?? ''
}));
